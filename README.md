...
本计划用于指导在index.html的visualHelperContainer下开发可视化助手，遵循"先整体后细节"原则，界面风格与主界面一致。

### 已实现内容
- 左右两侧主分区布局，左侧为数据上传/对话/输入，右侧为项目栏/画布/微调区，整体flex布局，响应式自适应
- 左右分区间、分区内部均支持拖动分割线调整比例，部分分割线为装饰分割线
- 画布区支持自适应大小、缩放、平移、选中与拖动图形等基础交互
- 左右对话区、输入区、标题栏、导出区等UI结构与主界面风格统一
- 画布区已预留图片渲染与json渲染能力的扩展接口
...
- 画布区：自适应canvas，支持缩放、拖动、选中、拖动图形等基础交互
...
### 阶段五：功能集成与交互完善（已完成）
- 当前优先目标：实现左右两侧对话功能并接入LLM API
  - 左侧对话区支持用户输入、系统/助手回复、气泡渲染、上下文管理
  - 右侧微调区支持对选中图形的代码微调对话
  - 接入SiliconFlow等大模型API，实现对话交互

### 阶段六：数据上传与预览区及后端智能体集成（进行中）
- 在可视化助手界面左上角实现"数据上传与预览区"
- 支持csv、excel文件拖拽或点击上传，上传后解析并预览前几行数据
- 预览区位于虚线框下方，样式与主界面统一
- 参考参考项目文件的数据上传与预览实现方式
- 对接后端API，实现：
  - 多智能体自动识别用户意图
  - 字段提取与数据理解
  - 自动生成并执行计算/绘图代码
  - 渲染图片（png/jpg）和json源码，返回并绑定到前端画布
  - 数据上传与预览联动
  - 画布区json渲染与交互
  - 对话区与后端API对接
  - 导出功能实现
